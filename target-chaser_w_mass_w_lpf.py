...
class PID: 
    """ PID Controller """
    def __init__(self, P, I, D, lambda_d=0.9, window_size=100):
        self.Kkp = P #Proportional term
        self.Kki = I #Integration term
        self.Kkd = D #Derivative term
        self.lambda_d = lambda_d  # Low-pass filter coefficient for derivative
        ...
        self.prev_derivative = 0  # Previous filtered derivative
        self.outputs = []  # Buffer to store output for moving average
    ...
    def update(self, setpoint, pv, dt):
        ...
        derivative = self.lambda_d * self.prev_derivative + (1 - self.lambda_d) * ((error - self.prev_error) / dt) # Low-pass filter on derivative
        self.prev_derivative = derivative  # Update the previous derivative
        output = self.Kkp * error + self.Kki * self.integral + self.Kkd * derivative #PID output calculation
        
        # Moving average filter on output
        self.outputs.append(output)
        if len(self.outputs) > self.window_size:
            self.outputs.pop(0)  # Remove the oldest output
        averaged_output = sum(self.outputs) / len(self.outputs)
